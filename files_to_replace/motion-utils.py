...
def reset_joints_to(
    ...
    if type(start_joint_pos) is list:
        action = start_joint_pos + [gripper_action * gripper_width]
    else:
        action = start_joint_pos.tolist() + [gripper_action * gripper_width]
    while True:
        if (
            ...
            if (
                np.max(
                    np.abs(np.array(robot_interface.last_q) -
                           np.array(start_joint_pos))
                )
                < 2e-3
            ):
                break
...
def joint_interpolation_traj(
    ...
    if traj_interpolator_type == "min_jerk":
        ...
        traj = np.array(
            [start_q + grid[i] * step_size for i in range(num_steps)])
