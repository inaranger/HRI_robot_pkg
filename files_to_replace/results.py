...
class Results(SimpleClass):
    ...
    def map_color(self, name, colors):
        if name == "yellow":
            return colors(7, True)
        elif name == "red":
            return colors(20, True)
        elif name == "blue":
            return colors(0, True)
        elif name == "green":
            return colors(13, True)
        elif name == "orange":
            return colors(18, True)
        else:
            return colors(2, True)
    ...
    def plot(
        ...
        if pred_masks and show_masks:
            ...
            color_array = [names[int(idx)].split('_')[-1] for idx in pred_boxes.cls]
            mapped_colors = [self.map_color(name, colors) for name in color_array]
            annotator.masks(pred_masks.data, colors=mapped_colors, im_gpu=im_gpu)
        ...
        if pred_boxes is not None and show_boxes:
            for d in reversed(pred_boxes):
                ...
                annotator.box_label(box, label, color=self.map_color(name.split('_')[-1], colors), rotated=is_obb)
